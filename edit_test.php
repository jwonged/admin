...
<html lang="en">
	<head>
		<meta charset="utf-8">
		<title>Edit Test Question | Objective Exam </title>
		...
		<script type="text/javascript">
			...
			function show_sub_question() 
			{
				...
				xmlhttp.open("GET","test.php?subjectid="+subId,true);
				xmlhttp.send();
			}
		</script>
		...
		<script type="text/javascript">
			function myFunction(testid,questionid) 
			{
				...
				{
					window.location = "delete_test_question.php?testid="+testid+"&question_id="+questionid;
				}
			}
			
			function add_questions(testid,subid)
			{ 
				window.location.href="add_question_in_test.php?test="+testid+"&sub="+subid;
			}
		</script>
	</head>
	<body>    
						...
						<form name="edit_test" action="" method="post" enctype="multipart/form-data" style="background-color:#f5f5f5;width: 1060px;margin-left: -228px;border-radius: 4px;" onsubmit="return(validate());">
							...
							<table style="margin-left: 15%;">
								...
								<tr style="background-color: white;">
									...
									<td style="background-color: #f5f5f5;">
										<input type="text" name="testname" value="<?php echo $row['name'] ?>" pattern="[\S]+[a-zA-Z\+-x*\s]+[0-9]*" title="Only enter letters" required>
									</td>
								</tr>	
								<tr>
									<td style="background-color: #f5f5f5;">
							...
							<div class="control-label"> 
								...
								<div class="controls" id="que" name="que" style="margin-left:243px;">
									<input type="button" id="clear" name="clear" value="Add Question" style="width: 220px;height: 40px;" class="btn btn-primary button-loading" data-loading-text="Loading..." onclick="add_questions('<?php echo $testid ?>','<?php echo $subjectid ?>');">
								</div>	
									...
									<?php
										...
										while($row4 = mysqli_fetch_assoc($result4))
										{
											...
											while($row1 = mysqli_fetch_assoc($result1))
											{
												...
												echo "<td align='center' style='width: 330px;height: 30px;'><a href='edit_test_questions.php?queid=".$row1['question_id']."&testid=".$testid."&subjectid=".$subjectid."' class='button edit'>Edit</a>&nbsp;<a href='#' class='button delete' onclick='myFunction(".$testid.",".$queid.");'>Delete</a></td>";
												echo "</tr>";
											}
										}	
		...
		<script type="text/javascript">
			function back()
			{ 
				window.location.href="test_details.php";
			}
		</script>
</html>